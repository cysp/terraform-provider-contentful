data "contentful_locales" "test" {
  ...
  lifecycle {
    postcondition {
      condition     = length([for locale in self.locales : locale if locale.code == "en-gb"]) == 1
...
locals {
  selected = one([for locale in data.contentful_locales.test.locales : locale if locale.code == "en-gb"])
}
