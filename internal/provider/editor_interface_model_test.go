...
func TestRoundTripToPutEditorInterfaceReq(t *testing.T) {
		...
		Editors: contentfulManagement.NewOptNilEditorInterfaceEditorsItemArray([]contentfulManagement.EditorInterfaceEditorsItem{
			{
				...
				WidgetId:        "widget_id",
				Disabled:        contentfulManagement.NewOptBool(false),
	...
	assert.True(t, req.Editors.Set)
	assert.Len(t, req.Editors.Value, 1)
	assert.Equal(t, contentfulManagement.PutEditorInterfaceReqEditorsItem{
		WidgetNamespace: "widget_namespace",
		WidgetId:        "widget_id",
		Disabled:        contentfulManagement.NewOptBool(false),
		Settings:        []byte(`{"foo":"bar"}`),
