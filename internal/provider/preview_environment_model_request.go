...
func (model *PreviewEnvironmentModel) ToPreviewEnvironmentData(_ context.Context, modelPath path.Path) (cm.PreviewEnvironmentData, diag.Diagnostics) {
	...
	keys := slices.Sorted(maps.Keys(configurations))
	...
	return cm.PreviewEnvironmentData{
		...
		Configurations: requestConfigurations,
	}, diagnostics
}
...
func (model *PreviewEnvironmentModel) ToPreviewEnvironmentUpdateData(
	...
	for _, contentTypeID := range slices.Sorted(maps.Keys(planConfigurations)) {
	...
	for _, contentTypeID := range slices.Sorted(maps.Keys(stateConfigurations)) {
