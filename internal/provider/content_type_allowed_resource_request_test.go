...
func TestContentTypeAllowedResourceRequiresExactlyOneAlternative(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			actual, diags := test.value.ToResourceLink(valuePath)
...
func TestContentTypeAllowedContentfulEntryPreservesKnownEmptyContentTypes(t *testing.T) {
	t.Parallel()

	actual, diags := ContentTypeFieldAllowedResourceItemContentfulEntryValue{
		Source:       types.StringValue("source"),
		ContentTypes: NewTypedList([]types.String{}),
	}.ToResourceLink(path.Root("allowed_resources").AtListIndex(0).AtName("contentful_entry"))
