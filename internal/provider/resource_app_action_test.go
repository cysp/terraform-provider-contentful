...
func TestAccAppActionResourceInvalidUpdate(t *testing.T) {
	t.Parallel()

	for _, test := range []struct{ name, initial, update, ignored, diagnostic string }{
		{"custom to builtin", actionLegacyConfig, actionBuiltinConfig, "parameters", "Built-in App Action parameters are read-only"},
		{"custom input switch", actionLegacyConfig, actionSchemaConfig, "parameters", "Invalid Custom App Action input"},
		{"ignored custom category", actionSchemaConfig, actionBuiltinConfig, "category", "Invalid Custom App Action input"},
		{"ignored builtin category", actionBuiltinConfig, actionLegacyConfig, "category", "Built-in App Action parameters are read-only"},
		{"ignored endpoint URL", actionLegacyConfig, actionFunctionConfig, "url", "Invalid App Action configuration"},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()

			lifecycle := "\n lifecycle { ignore_changes = [" + test.ignored + "] }\n}"
			config := strings.Replace(test.initial, "\n}", lifecycle, 1)
			update := strings.Replace(test.update, "\n}", lifecycle, 1)
			server, err := cmt.NewContentfulManagementServer(cmt.WithRateLimitPerSecond(1000))
			require.NoError(t, err)
			server.SetAppDefinition("org", "app", cm.AppDefinitionData{Name: "App"})
			...
			testAccMockedResource(t, handler, resource.TestCase{
				...
				Steps: []resource.TestStep{
					{Config: config},
					{Config: update, PlanOnly: true, ExpectNonEmptyPlan: true},
					{Config: update, ExpectError: regexp.MustCompile(test.diagnostic)},
			...
			assert.EqualValues(t, 2, writes.Load(), "only creation and the corrected update may mutate the action")
		})
	}
}

func TestAccAppActionResourceCategoryReplacement(t *testing.T) {
	t.Parallel()

	server, err := cmt.NewContentfulManagementServer(cmt.WithRateLimitPerSecond(1000))
	require.NoError(t, err)
	server.SetAppDefinition("org", "app", cm.AppDefinitionData{Name: "App"})
	server.SetAppDefinition("org", "other-app", cm.AppDefinitionData{Name: "Other App"})

	config := strings.Replace(actionLegacyConfig, "\n}", "\n lifecycle { ignore_changes = [parameters] }\n}", 1)
	update := strings.Replace(actionBuiltinConfig, "\n}", "\n lifecycle { ignore_changes = [parameters] }\n}", 1)
	update = strings.Replace(update, `app_definition_id = "app"`, `app_definition_id = "other-app"`, 1)
	testAccMockedResource(t, server, resource.TestCase{Steps: []resource.TestStep{
		{Config: config},
		{Config: update, ConfigPlanChecks: resource.ConfigPlanChecks{PreApply: []plancheck.PlanCheck{
			plancheck.ExpectResourceAction(actionAddress, plancheck.ResourceActionDestroyBeforeCreate),
		}}, ConfigStateChecks: []statecheck.StateCheck{
			statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("category"), knownvalue.StringExact("Entries.v1.0")),
			statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("parameters"), knownvalue.Null()),
		}},
	}})
}

func TestAccAppActionResourceTriggeredReplacement(t *testing.T) {
	t.Parallel()

	for _, test := range []struct{ name, update, ignored string }{
		{"category", actionBuiltinConfig, "parameters"},
		{"input", actionSchemaConfig, "parameters"},
		{"executor", actionFunctionConfig, "url"},
	} {
		for _, order := range []struct {
			...
			{"destroy before create", false, plancheck.ResourceActionDestroyBeforeCreate, []string{http.MethodPost, http.MethodDelete, http.MethodPost, http.MethodDelete}},
			{"create before destroy", true, plancheck.ResourceActionCreateBeforeDestroy, []string{http.MethodPost, http.MethodPost, http.MethodDelete, http.MethodDelete}},
		} {
			t.Run(test.name+"/"+order.name, func(t *testing.T) {
				...
				lifecycle := fmt.Sprintf("\n lifecycle {\n ignore_changes = [%s]\n replace_triggered_by = [terraform_data.trigger]\n create_before_destroy = %t\n }\n}", test.ignored, order.createBeforeDestroy)
				initial := strings.Replace(actionLegacyConfig, "\n}", lifecycle, 1) + "\nresource \"terraform_data\" \"trigger\" { input = 1 }"
				update := strings.Replace(test.update, "\n}", lifecycle, 1) + "\nresource \"terraform_data\" \"trigger\" { input = 2 }"
				testAccMockedResource(t, handler, resource.TestCase{Steps: []resource.TestStep{
					{Config: initial},
					{Config: update, ConfigPlanChecks: resource.ConfigPlanChecks{PreApply: []plancheck.PlanCheck{
						plancheck.ExpectResourceAction(actionAddress, order.action),
					}}},
				}})
				requestMutex.Lock()
				defer requestMutex.Unlock()

				assert.Equal(t, order.methods, methods)
			})
		}
	}
}

func TestAccAppActionResourceBuiltinParameters(t *testing.T) {
	t.Parallel()
	...
	const discovery = `
data "contentful_app_action" "builtin" {
 ...
 depends_on = [contentful_app_action.test]
}`
	...
	for _, test := range []struct{ config, definitions string }{
		...
		var definitions knownvalue.Check = knownvalue.Null()
		if test.definitions != "" {
			definitions = knownvalue.StringExact(test.definitions)
		}

		steps = append(steps, resource.TestStep{Config: config, ConfigStateChecks: []statecheck.StateCheck{
			statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("parameters"), knownvalue.Null()),
			statecheck.ExpectKnownValue("data.contentful_app_action.builtin", tfjsonpath.New("parameters"), definitions),
		}})
	}
...
func TestAccAppActionResourceCategoryDrift(t *testing.T) {
	t.Parallel()

	for _, test := range []struct {
		name, config, category string
		parameters             []byte
		...
		t.Run(test.name, func(t *testing.T) {
			...
			testAccMockedResource(t, server, resource.TestCase{Steps: []resource.TestStep{
				{Config: test.config},
				{Config: test.config, PreConfig: func() {
					response, err := server.Handler().GetAppActions(t.Context(), cm.GetAppActionsParams{OrganizationID: "org", AppDefinitionID: "app"})
					require.NoError(t, err)
					...
					require.True(t, ok)
					require.Len(t, actions.Items, 1)
					updated, err := server.Handler().UpdateAppAction(t.Context(), &cm.AppActionData{
						Name: "Action", Category: test.category, Type: "endpoint", URL: cm.NewOptString("https://example.invalid/action"), Parameters: test.parameters,
					}, cm.UpdateAppActionParams{OrganizationID: "org", AppDefinitionID: "app", AppActionID: actions.Items[0].Sys.ID})
					require.NoError(t, err)
					require.IsType(t, &cm.AppAction{}, updated)
				}, ConfigPlanChecks: resource.ConfigPlanChecks{PreApply: []plancheck.PlanCheck{
					plancheck.ExpectResourceAction(actionAddress, plancheck.ResourceActionUpdate),
					testAccPriorState{check: statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("category"), knownvalue.StringExact(test.category))},
					testAccPriorState{check: statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("parameters"), test.refreshed)},
				}}, ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(actionAddress, tfjsonpath.New("parameters"), test.restored),
				}},
			}})
		})
	}
...
func TestAccAppActionResourceUnknownValues(t *testing.T) {
	t.Parallel()

	const prefix = `resource "contentful_app_action" "test" {
 organization_id = "org"
 app_definition_id = "app"
 name = "Action"
 category = terraform_data.definition.output.category
 type = terraform_data.definition.output.type
 url = terraform_data.definition.output.url
 function_id = terraform_data.definition.output.function_id
 parameters = terraform_data.definition.output.parameters
 parameters_schema = terraform_data.definition.output.parameters_schema
}
resource "terraform_data" "definition" {
 input = `

	cases := []actionMutationFixture{
		{prefix + `{ category="Custom", type="endpoint", url="https://example.invalid/action", function_id=null, parameters=jsonencode([]), parameters_schema=null }
}`, actionLegacyBody},
		{prefix + `{ category="Custom", type="endpoint", url="https://example.invalid/action", function_id=null, parameters=null, parameters_schema=jsonencode({type="object", properties={message={type="string"}}}) }
}`, actionSchemaBody},
	...
	testActionLifecycle(t, cases,
		...
		plancheck.ExpectUnknownValue(actionAddress, tfjsonpath.New("type")),
		plancheck.ExpectUnknownValue(actionAddress, tfjsonpath.New("url")),
		plancheck.ExpectUnknownValue(actionAddress, tfjsonpath.New("function_id")),
		plancheck.ExpectUnknownValue(actionAddress, tfjsonpath.New("parameters")),
		plancheck.ExpectUnknownValue(actionAddress, tfjsonpath.New("parameters_schema")),
	)
}

func TestAccAppActionResourceInvalidResolvedInput(t *testing.T) {
	t.Parallel()
	...
	config := strings.Replace(actionLegacyConfig, `category = "Custom"`, `category = terraform_data.category.output`, 1) + "\nresource \"terraform_data\" \"category\" { input = \"Entries.v1.0\" }"
	testAccMockedResource(t, handler, resource.TestCase{Steps: []resource.TestStep{
		{Config: config, PlanOnly: true, ExpectNonEmptyPlan: true},
		{Config: config, ExpectError: regexp.MustCompile("Built-in App Action parameters are read-only")},
	}})
