...
func NewAppKeyResourceModelFromResponse(_ context.Context, appKey cm.AppKey, privateKey types.String) (AppKeyModel, diag.Diagnostics) {
	...
	model.JWK = NewTypedObject(NewAppKeyJWKModelFromResponse(appKey.Jwk))

	model.PrivateKey = privateKey
	if responsePrivateKey, ok := appKeyPrivateKeyFromResponse(appKey); ok {
		model.PrivateKey = types.StringValue(responsePrivateKey)
...
func appKeyPrivateKeyFromResponse(appKey cm.AppKey) (string, bool) {
	if generated, ok := appKey.Generated.Get(); ok {
		return generated.PrivateKey, true
	}

	return appKey.PrivateKey.Get()
}

func NewAppKeyJWKModelFromResponse(jwk cm.AppKeyJWK) AppKeyJWKModel {
	x5cElements := make([]types.String, 0, len(jwk.X5c))
	for _, certificate := range jwk.X5c {
		x5cElements = append(x5cElements, types.StringValue(certificate))
	}
