...
func (r *appActionResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	if !config.Category.IsNull() && !config.Category.IsUnknown() && config.Category.ValueString() != "Custom" && !config.Parameters.IsNull() && !config.Parameters.IsUnknown() {
		resp.Diagnostics.AddAttributeError(path.Root("parameters"), "Built-in App Action parameters are read-only", "Only Custom actions accept configured parameters. Omit parameters for other categories; Contentful supplies their definitions.")
	}

	if config.Category.ValueString() == "Custom" && !config.Parameters.IsUnknown() && !config.ParametersSchema.IsUnknown() && config.Parameters.IsNull() == config.ParametersSchema.IsNull() {
		resp.Diagnostics.AddAttributeError(path.Root("parameters"), "Invalid Custom App Action input", "For category Custom, configure either parameters or parameters_schema, but not both.")
	}
}

func (r *appActionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var plan, config AppActionModel
...
func (r *appActionResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	base, identityDiags := appActionResponse(*result, state.AppActionBaseModel)
...
func (r *appActionResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan, config, state AppActionModel
	resp.Diagnostics.Append(req.Plan.Get(ctx, &plan)...)
	resp.Diagnostics.Append(req.Config.Get(ctx, &config)...)
	resp.Diagnostics.Append(req.State.Get(ctx, &state)...)
	...
	// Only unconfigured computed parameters may adopt state for this comparison.
	comparison := plan
	if comparison.Parameters.IsUnknown() && config.Parameters.IsNull() {
		comparison.Parameters = state.Parameters
	}

	if appActionRemoteValuesEqual(ctx, comparison, state) {
...
func appActionRemoteValuesEqual(ctx context.Context, plan, state AppActionModel) bool {
	// Compare JSON semantically, then use the existing model equality for every
	// remaining attribute. Timeouts are local and excluded by the base model.
