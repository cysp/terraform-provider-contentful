...
func (model *AppDefinitionParameters) ToAppDefinitionParameters(valuePath path.Path) (cm.AppDefinitionParameters, diag.Diagnostics) {
	installation, installationDiags := appDefinitionParameterListToRequest(valuePath.AtName("installation"), model.Installation)
	instance, instanceDiags := appDefinitionParameterListToRequest(valuePath.AtName("instance"), model.Instance)
...
func appDefinitionParameterListToRequest(valuePath path.Path, models []AppDefinitionParameter) ([]cm.AppDefinitionParameter, diag.Diagnostics) {
	if models == nil {
		return nil, nil
	}
	...
	for index, model := range models {
		parameter, parameterDiags := model.ToAppDefinitionParameter(valuePath.AtListIndex(index))
...
func (model AppDefinitionParameter) ToAppDefinitionParameter(parameterPath path.Path) (cm.AppDefinitionParameter, diag.Diagnostics) {
	diags := diag.Diagnostics{}
