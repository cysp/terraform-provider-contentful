...
func (r *previewEnvironmentResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	params := cm.DeletePreviewEnvironmentParams{
		SpaceID:              state.SpaceID.ValueString(),
		PreviewEnvironmentID: state.PreviewEnvironmentID.ValueString(),
	}

	response, err := r.providerData.client.DeletePreviewEnvironment(ctx, params)

	tflog.Info(ctx, "preview_environment.delete", map[string]any{
		"params":   params,
		"response": response,
		"err":      err,
	})
