...
func TestAppActionCreatedIdentityRemainsGenerated(t *testing.T) {
	t.Parallel()

	plan := appActionTestModel()
	response := appActionTestResponse()
	response.Sys.ID = "generated"
	data, consistency := reconcileAppActionResponse(t.Context(), response, plan, AppActionBaseModel{OrganizationID: plan.OrganizationID, AppDefinitionID: plan.AppDefinitionID})
	require.True(t, consistency.HasError())
	assert.Equal(t, "org/app/generated", data.ID.ValueString())
	assert.Equal(t, "generated", data.AppActionID.ValueString())
}
