...
func (model AppActionModel) ToAppActionData(config AppActionModel) (cm.AppActionData, diag.Diagnostics) {
	...
	// ignore_changes can retain a built-in category for a Custom configuration.
	// Allow that plan, but reject configured parameters for built-in configurations.
	if category != "Custom" && config.Category.ValueString() != "Custom" && !config.Parameters.IsNull() {
		diags.AddAttributeError(path.Root("parameters"), "Built-in App Action parameters are read-only", "Omit parameters for built-in categories; Contentful supplies their definitions.")
	}
	// Only Custom parameters are writable. Known planned parameters still constrain
	// the response for every category; unconfigured, unknown parameters may be omitted.
	if category == "Custom" && (!model.Parameters.IsUnknown() || !config.Parameters.IsNull()) {
	...
	if data.Category == "Custom" && (len(data.Parameters) != 0) == (len(data.ParametersSchema) != 0) {
		diags.AddAttributeError(path.Root("parameters"), "Invalid Custom App Action input", "For category Custom, configure either parameters or parameters_schema, but not both.")
	}
...
// Both configuration validation and request conversion use the same target
// rules. Unknown configuration values defer only the checks that depend on them.
