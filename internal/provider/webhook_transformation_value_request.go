...
func ToOptNilWebhookDefinitionDataTransformation(valuePath path.Path, value TypedObject[WebhookTransformationValue]) (cm.OptNilWebhookDefinitionDataTransformation, diag.Diagnostics) {
	...
	default:
		...
		method, methodDiags := requestOmittableString(value.Method, valuePath.AtName("method"))
		diags.Append(methodDiags...)

		contentType, contentTypeDiags := requestOmittableString(value.ContentType, valuePath.AtName("content_type"))
		diags.Append(contentTypeDiags...)

		includeContentLength, includeContentLengthDiags := requestOmittableBool(
