...
func TestSetResourceIdentityAndStateDoesNotPublishAfterExistingError(t *testing.T) {
	t.Parallel()

	identity, state := emptyPublicationTargets()
	existingDiagnostics := diag.Diagnostics{
		diag.NewErrorDiagnostic("Earlier conversion failed", "State must remain untouched."),
	}
