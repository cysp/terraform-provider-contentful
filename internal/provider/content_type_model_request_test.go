...
func TestContentTypeRequestRejectsUnresolvedValues(t *testing.T) {
	t.Parallel()
...
func TestContentTypeRequestPreservesDescriptionPresence(t *testing.T) {
	t.Parallel()
	...
	for name, test := range map[string]struct {
		...
		"non-empty": {
			...
			expected:    cm.NewOptNilString("A test content type"),
		},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
...
func TestContentTypeFieldsRejectUnresolvedElementsWithoutPartialOutput(t *testing.T) {
	...
	assert.Nil(t, result)
	assert.Equal(t, []string{"fields[1]", "fields[2]"}, attributeDiagnosticPaths(t, diags))
}
...
func TestContentTypeValidationsFailClosedWithExactPaths(t *testing.T) {
	...
	assert.Nil(t, actual)
	assert.Equal(t, []string{"fields[0].validations[1]", "fields[0].validations[2]"}, attributeDiagnosticPaths(t, diags))
}
