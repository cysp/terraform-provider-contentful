...
func TestReconcilePreviewEnvironmentMutationResponseHonorsSelectedIDOwnership(t *testing.T) {
	...
	generatedPlan := plan
	generatedPlan.ID = types.StringUnknown()
	generatedPlan.PreviewEnvironmentID = types.StringUnknown()
	...
	constrainedState, responseDiagnostics, consistencyDiagnostics := ReconcilePreviewEnvironmentMutationResponse(
		t.Context(), response, plan,
		PreviewEnvironmentIdentityModel{SpaceID: plan.SpaceID, PreviewEnvironmentID: types.StringNull()},
	)
	assert.Empty(t, responseDiagnostics)
	require.True(t, consistencyDiagnostics.HasError())
	assert.Equal(t, []string{"preview_environment_id", "id"}, attributeDiagnosticPaths(t, consistencyDiagnostics))
	assert.Equal(t, types.StringValue("generated-id"), constrainedState.PreviewEnvironmentID)
	assert.Equal(t, types.StringValue("space/generated-id"), constrainedState.ID)
...
func TestReconcilePreviewEnvironmentMutationResponseRejectsInconsistentLegacyID(t *testing.T) {
	t.Parallel()

	plan := previewEnvironmentMutationModel("preview", "", map[string]string{})
	plan.ID = types.StringValue("space/other-preview")
	response := previewEnvironmentResponse("space", "preview", "Planned", "", map[string]string{})

	state, responseDiags, consistencyDiags := ReconcilePreviewEnvironmentMutationResponse(t.Context(), response, plan, plan.PreviewEnvironmentIdentityModel)

	assert.Empty(t, responseDiags)
	require.True(t, consistencyDiags.HasError())
	assert.Equal(t, []string{"id"}, attributeDiagnosticPaths(t, consistencyDiags))
	assert.Equal(t, types.StringValue("space/preview"), state.ID)
	assert.Equal(t, types.StringValue("preview"), state.PreviewEnvironmentID)
}

func TestReconcilePreviewEnvironmentMutationResponseUsesResponseForUnknownPlan(t *testing.T) {
	t.Parallel()

	plan := previewEnvironmentMutationModel("preview", "", map[string]string{})
	plan.Name = types.StringUnknown()
	plan.Description = types.StringUnknown()
	plan.ContentTypeConfigurations = NewTypedMapUnknown[TypedObject[PreviewEnvironmentContentTypeConfigurationValue]]()
	response := previewEnvironmentResponse("space", "preview", "Returned", "remote description", map[string]string{"page": "https://preview.invalid/page"})
	...
	assert.Equal(t, types.StringValue("remote description"), state.Description)
	assert.True(t, previewEnvironmentModel(map[string]string{"page": "https://preview.invalid/page"}).ContentTypeConfigurations.Equal(state.ContentTypeConfigurations))
}

func TestReconcilePreviewEnvironmentMutationResponseDistinguishesNullAndEmptyMap(t *testing.T) {
	t.Parallel()

	plan := previewEnvironmentMutationModel("preview", "", map[string]string{})
	plan.ContentTypeConfigurations = NewTypedMapNull[TypedObject[PreviewEnvironmentContentTypeConfigurationValue]]()
	response := previewEnvironmentResponse("space", "preview", "Planned", "", map[string]string{})
	...
	assert.Empty(t, responseDiags)
	require.True(t, consistencyDiags.HasError())
	assert.Equal(t, []string{"content_type_configurations"}, attributeDiagnosticPaths(t, consistencyDiags))
	assert.False(t, state.ContentTypeConfigurations.IsNull())
	assert.Empty(t, state.ContentTypeConfigurations.Elements())
}
