...
func TestEditorLayoutGroupItemPreservesFieldItemAsKnownNull(t *testing.T) {
	...
	assert.Equal(t, []string{"editor_layout"}, attributeWarningPaths(t, diags))
...
func TestEditorLayoutNestedGroupItemFieldPreservesGroupItemAsKnownNull(t *testing.T) {
	...
	assert.False(t, diags.HasError())
	assert.Len(t, diags.Warnings(), 1)
	assert.Equal(t, []string{"items"}, attributeWarningPaths(t, diags))
	assert.True(t, actual.IsNull())
}
...
func TestEditorLayoutPreservesUnknownItemTypesAndListPositions(t *testing.T) {
	...
	assert.Equal(t, []string{itemPath.String()}, attributeWarningPaths(t, groupItemDiags))
	...
	assert.Equal(t, []string{itemPath.String()}, attributeWarningPaths(t, topLevelGroupDiags))
	...
	assert.Equal(t, []string{itemPath.String()}, attributeWarningPaths(t, nestedFieldDiags))
	...
	assert.Equal(t, []string{itemPath.String()}, attributeWarningPaths(t, layoutDiags))
