...
func (r *tagResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	params := cm.CreateTagParams{
		SpaceID:                  plan.SpaceID.ValueString(),
		EnvironmentID:            plan.EnvironmentID.ValueString(),
		XContentfulTagVisibility: util.StringValueToOptString(plan.Visibility),
	}

	request, requestDiags := plan.ToTagData(ctx)
	...
	response, err := r.providerData.client.CreateTag(ctx, &request, params)
	...
	if tagStatus, ok := response.(*cm.TagStatusCode); ok {
		resp.Diagnostics.Append(SetPrivateProviderData(ctx, resp.Private, "version", tagStatus.Response.Sys.Version)...)
	}
}
...
func (r *tagResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	if tag != nil {
		resp.Diagnostics.Append(SetPrivateProviderData(ctx, resp.Private, "version", tag.Sys.Version)...)
	}
}
...
func (r *tagResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	default:
		if response, ok := response.(cm.StatusCodeResponse); ok {
			if response.GetStatusCode() == http.StatusNotFound {
				resp.Diagnostics.AddWarning("Failed to delete tag", util.ErrorDetailFromContentfulManagementResponse(response, err))
				return
			}
		}
