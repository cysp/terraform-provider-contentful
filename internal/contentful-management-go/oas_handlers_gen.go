...
func (s *Server) handleCreateTagRequest(args [2]string, argsEscaped bool, w http.ResponseWriter, r *http.Request) {
	statusWriter := &codeRecorder{ResponseWriter: w}
	w = statusWriter
	ctx := r.Context()
	...
	)
	{
		...
		var satisfied bitset
		{
			sctx, ok, err := s.securityAccessToken(ctx, CreateTagOperation, r)
			if err != nil {
				err = &ogenerrors.SecurityError{
					OperationContext: opErrContext,
					Security:         "AccessToken",
