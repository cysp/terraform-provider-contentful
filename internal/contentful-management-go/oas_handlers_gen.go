...
func (s *Server) handleGetTagsRequest(args [2]string, argsEscaped bool, w http.ResponseWriter, r *http.Request) {
	statusWriter := &codeRecorder{ResponseWriter: w}
	w = statusWriter
	ctx := r.Context()
	...
	)
	{
		...
		var satisfied bitset
		{
			sctx, ok, err := s.securityAccessToken(ctx, GetTagsOperation, r)
			if err != nil {
				err = &ogenerrors.SecurityError{
					OperationContext: opErrContext,
					Security:         "AccessToken",
	...
	params, err := decodeGetTagsParams(args, argsEscaped, r)
	if err != nil {
		err = &ogenerrors.DecodeParamsError{
			OperationContext: opErrContext,
			Err:              err,
	...
	if m := s.cfg.Middleware; m != nil {
		mreq := middleware.Request{
			...
			OperationSummary: "Get a collection of tags",
			OperationID:      "getTags",
			Body:             nil,
			RawBody:          rawBody,
			Params: middleware.Parameters{
		...
		response, err = middleware.HookMiddleware[
			...
			unpackGetTagsParams,
			func(ctx context.Context, request Request, params Params) (response Response, err error) {
				response, err = s.h.GetTags(ctx, params)
				return response, err
			},
		)
	} else {
	...
	if err := encodeGetTagsResponse(response, w); err != nil {
		defer recordError("EncodeResponse", err)
		if !errors.Is(err, ht.ErrInternalServerErrorResponse) {
			s.cfg.ErrorHandler(ctx, w, r, err)
		}
