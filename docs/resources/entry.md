...
- After an Update writes a draft but publication is not confirmed, Terraform preserves the draft state but an unchanged later apply does not retry publication. Inspect Contentful, then publish manually if needed or make another Terraform-managed draft change.
