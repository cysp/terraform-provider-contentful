...
description: |-
  ...
  Updates and deletes can overwrite or remove changes made outside Terraform. Create, Update, and Delete are not automatically retried, including after rate limiting. If creation fails, check the App Definition for an action created without saved Terraform state and import it before applying again to avoid duplicates. After a failed update or delete, refresh state and review the plan before retrying.
---
...
- `type` (String) Action type: `endpoint` with `url`, or `function-invocation` with `function_id`.
...
- `parameters` (String) Legacy parameter definitions encoded as a JSON array. Each definition requires `id`, `name`, and `type` (`Boolean`, `Symbol`, `Number`, or `Enum`). For `Custom`, configure this or `parameters_schema`; `[]` defines no arguments. Omit for built-in categories; Contentful supplies their definitions.
- `parameters_schema` (String) Input JSON Schema (draft 4) object encoded with `jsonencode(...)`. Custom actions cannot also configure `parameters`. Omission removes the schema on update.
