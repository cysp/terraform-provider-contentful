...
After a Create or Update returns a complete, valid draft for the expected Content Type that agrees with the Terraform plan, the provider records only the exact positive returned `version` as eligible for automatic activation recovery. It does not assume a particular version increment. If activation is not confirmed, an unchanged later apply can activate that same version without repeating the Content Type PUT.
