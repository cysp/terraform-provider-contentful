...
| Preview environment configuration | The [preview environment contract](content-preview-environments.md) records direct CMA and Web App observations; the HTTP endpoints are undocumented. | The fake merges configurations by content type identity, retains omitted and disabled configurations, and advances the version only for metadata changes. | Selected-ID deletion residue and delayed read-after-delete visibility are not modeled. Version locking cannot detect concurrent configuration-only changes. | Client tests cover merge, disable, re-enable, identity aliases, and version conflicts; provider tests cover exact delta requests, projection, recovery state, import, and lifecycle transitions. Live observations remain dated evidence, not a guarantee. |
