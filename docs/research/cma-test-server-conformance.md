...
| --- | --- | --- | --- | --- |
| Space Enablements PUT | [CMA Space Enablements reference](https://www.contentful.com/developers/docs/references/content-management-api/space-enablements/) requires `spaceTemplates` and `crossSpaceLinks` to have equal booleans; the sanitized [Space Enablements request-values probe](space-enablement-values.md) observed that CMA also rejects either missing member and leaves the existing document unchanged | The fake resolves the space and applies the version fence before validating that both paired members are present and equal. Invalid requests return 422 `ValidationFailed` without storing a new document, changing any member, or advancing `sys.version`. | Exact CMA validation details were not retained and remain deliberately unmodelled; the fake uses a nonempty generic message. All four Terraform attributes remain independently Optional+Computed: the provider forwards known effective-plan values without inferring or locally validating a sibling. | Direct handler tests cover omitted, one-sided, unequal, missing-space, stale-version, absent-document, nonmutation, and exact valid version advancement. Mocked Terraform lifecycle tests prove exact forwarded bodies, surfaced validation failures, failed-Update truthfulness, recovery, and an empty converged plan. |
