...
| Webhook topics | Contentful's [Webhook configuration documentation](https://www.contentful.com/developers/docs/extensibility/webhooks/configure-webhook/#topics) requires at least one topic. Sanitized direct Create and Update observations established that omitted and empty topics return 422 `ValidationFailed` without creating or changing a Webhook. | The fake returns the observed missing-topic and empty-topic validation details before storing either Create or Update requests. | The fake previously returned a generated 400 decoder error for omitted topics and accepted empty topics. The provider now requires a non-empty list while still deferring unknown values during validation. The fake's pre-existing successful Create status is outside this topics-validation correction. | Raw HTTP tests cover exact POST and PUT paths, status and response shapes, and failed-request nonmutation. Mocked Terraform lifecycle tests cover local omission and emptiness rejection before mutation plus one-topic acceptance. |
| Webhook Basic password | Contentful's [Webhook configuration documentation](https://www.contentful.com/developers/docs/extensibility/webhooks/configure-webhook/#http-basic-authentication) defines paired username/password writes and paired-null removal. Sanitized direct Create, Update, and GET observations established that CMA accepts the password but omits `httpBasicPassword` from every response while retaining `httpBasicUsername`. | The fake stores the submitted password so later request behavior remains testable, but omits it from Create, Update, and GET responses. | The fake previously returned the password and masked Terraform's post-apply consistency failure. The provider restores planned or prior state only when the response property is absent; explicit JSON null remains distinct. | Raw-CMA JSON and Terraform lifecycle tests cover the omission, refresh, update, rotation, removal, import, and `ignore_changes`. |
