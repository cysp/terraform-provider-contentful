...
This document records the final adversarially validated findings from checking [`contentful-cma-manageable-settings.md`](contentful-cma-manageable-settings.md) against Contentful's current first-party API references, first-party product documentation, live web-app behavior, and the provider code and OpenAPI description in this checkout.
...
**Verdict: Unsupported as conventional CRUD.** An organization membership **cannot be created directly**; an organization invitation creates a pending membership as a side effect. List, get, update, and delete are documented. See [User Management API — Organization Memberships](https://www.contentful.com/developers/docs/references/user-management-api/#organization-memberships).

A conventional create/read/update/delete membership resource cannot implement `Create` against the membership collection. It would need to model an invitation and its transition to membership, or support only import/update/delete of an existing membership.
...
The live update and rollback of an existing preview configuration establishes that the web app currently uses the first endpoint with optimistic versioning. It does not prove supported personal-access-token authentication, create/delete behavior, long-term compatibility, or permission semantics.
...
The live `access_policy` request is an organization-scoped `GET`. Its object exposes fields for SSO enforcement, MFA enforcement, explicit token authorization, token-expiration limits, SCIM-only management, and granular environment policies. This establishes a broad organization access-policy read shape, not a narrow granular-permissions resource.
...
The Home and Content preview panes both requested that path, confirming that UI Config is environment-scoped. The separately observed shortened primary-environment route remains undocumented.
...
No public endpoint for changing the durable protection mode was located in the current CMA reference. Durable mode configuration and asset keys are separate concerns, and product documentation alone is not Tier A API evidence.
...
### Low: locale, direct space membership, and aggregate space members have distinct lifecycles
...
This readiness ordering weighs public support, lifecycle completeness, blast radius, sensitive state, importability, concurrency, and shared-document ownership:
...
The ordering is a design recommendation rather than an API fact. Before implementation, score candidates against the same criteria and likely user demand.
