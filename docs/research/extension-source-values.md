...
An Extension request must contain exactly one source. An empty `srcdoc` is a
valid source; an empty `src` is not. This note records the evidence for the
provider's [source-selection contract](../design/terraform-value-semantics.md#extension-sources).
...
explicit empty `src` is rejected by configuration validation;
