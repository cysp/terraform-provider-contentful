...
four data sources, two list resources, the shared import path, and every custom
...
| Resource: Delivery API Key | Name, description, environment-link list | Primitive fields are CRUD-known. Optional-computed environment links omit an unknown initial-create value; null/unknown children are rejected. Update plans preserve prior state. |
...
| Data source: Environment Status Ready | Space/environment IDs and environment-status response | Required inputs are known for Read. The response conversion and `State.Set` share an explicit error barrier, leaving response state unset on failure. |
...
| List resource: Content Type | Space/environment list configuration, identity and optional resource result | Required IDs use known-value extraction before API access. Identity conversion is required; optional resource result data is set only after response conversion succeeds. |
| List resource: Entry | Space/environment IDs, optional content type, order, query, identity and optional resource result | Optional values distinguish null from unknown. Order/query children are checked with exact paths before API access. Result resource data remains unset on conversion failure and is not managed-resource state. When a full API item omits `fields`, the list result normalizes that required resource attribute to a known empty map. |
...
| Delivery API Key `environments` | Optional+Computed typed string list; `NoNullValues`; state-for-unknown | Rejected by schema | Unknown create value is omission; known children are checked at exact indices and errors return no environment links. |
...
| Delivery API Key `environments` | List, 1 | Optional+Computed managed collection. Omitted update configuration preserves prior environment links, including null; explicit empty remains a clear operation. |
...
component rejection, atomic managed identity/state publication, schema validation
that rejects null children while deferring unknown containers, named exact-one
request union conversion, fail-closed response unions, and response-conversion
