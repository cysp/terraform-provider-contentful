...
With the [provider configured](../index), save this example as `query.tfquery.hcl`. Replace `SPACE_ID` with your existing space ID. It finds Entries of the `blogPost` Content Type in the `master` environment:
...
Initialize the configuration, check the query, and inspect the matching identities:
...
Once the results match the objects you intend to manage, generate resource and import blocks into a new file:

```shell
terraform query -generate-config-out=generated.tf
```

See HashiCorp's [bulk import workflow](https://developer.hashicorp.com/terraform/language/import/bulk) for query syntax and configuration-generation options.

## 3. Review and apply the import
