...
1. Resolve and record the exact `main` commit intended for the release before
   publishing.
2. In GitHub, create a release named for the next `vMAJOR.MINOR.PATCH` tag,
   choose to create that same tag on publish, target `main`, write the release
   notes, and click **Publish release**.
3. Verify that the new tag resolves to the recorded release commit. Publishing
   the release creates the tag and starts the release workflow. The workflow
   requires an existing published release whose name and tag both match that
   version.
4. The protected `release` environment supplies the GPG signing key. GoReleaser
   builds once, signs the checksum file, and uploads the artifacts directly to
   the existing release.
5. The workflow verifies the checksums and GPG signature, creates GitHub
...
Set the published tag once, download every asset, then verify its checksum and
the detached GPG signature using the maintainer public key obtained through a
trusted channel:
...
gh release download "$VERSION" --repo cysp/terraform-provider-contentful
shasum -a 256 --check "terraform-provider-contentful_${RELEASE_VERSION}_SHA256SUMS"
gpg --verify "terraform-provider-contentful_${RELEASE_VERSION}_SHA256SUMS.sig" \
  "terraform-provider-contentful_${RELEASE_VERSION}_SHA256SUMS"
```
